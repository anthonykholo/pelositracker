...
An API-first, paper-only live sports monitoring prototype. Register an event, stream score and market updates, and receive explainable signals gated by freshness, source agreement, spread, estimated edge, and a configurable signal-quality threshold. The scoring engine is implemented in Rust and loaded by Python as a native extension.
...
- Lets you remove any monitored event from its card, cancelling its feed tasks and clearing its in-memory data.
...
The checked-in build helper initializes Microsoft's C++ linker environment before invoking Maturin. A normal server start does not rebuild Rust; rerun `build-rust.cmd` only after changing `native_engine/src/lib.rs`.
...
### Dashboard number guide

- **Recommendation**: `WATCH` means at least one safety threshold failed; `PAPER BET` means every configured gate passed. Neither places a wager.
- **Model probability**: the engine's estimated probability for that outcome after consensus normalization and the capped momentum adjustment.
- **Estimated edge**: model probability minus the best executable market probability. Positive is favorable according to the model; negative is not.
- **Signal quality**: a 0–100 data-quality score using freshness, source agreement/count, spread, and edge strength. It is not a predicted win rate.

Expanded **Why this signal?** panels remain open while live data refreshes.
